...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, m1,m2, gd in zip(
                self.params, self.moms_1, self.moms_2,grads_wrt_params):
            m1=self.beta_1*m1+(1-self.beta_1)*gd
            m2=self.beta_2*m2+(1-self.beta_2)*gd*gd
            m1_e=m1/(1-self.beta_1**self.step_count)
            m2_e=m2/(1-self.beta_2**self.step_count)
            param-=self.learning_rate*m1_e/(m2_e**0.5+self.epsilon)
...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def reset(self):
        ...
        the first and second moments of the gradients.
        """
        for mom1 in zip(self.moms_1):
            mom1*= 0.
        for mom2 in zip(self.moms_2):
            mom2 *= 0.
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, m1,m2, gd in zip(
            ...
            m1_e=m1/(1-self.beta_1**(self.step_count))
            m2_e=m2/(1-self.beta_2**(self.step_count))
            aa = self.learning_rate / self.initial_learning_rate
            param-=aa*(self.learning_rate*m1_e/(m2_e**0.5+self.epsilon)+self.weight_decay*param)
